    ...
    public class DesktopBar : VideoControllerEventListener
    {
        ...
        public void ValidateView()
        {
            ...
            if (core.isPlaying)
            {
                ...
                {
                    ratio = (float)texture.width / (float)texture.height;//宽/高，比如我现在这个是手机，那就是9:16
                    var screen = _canvas2Rect.sizeDelta;//获取画布的宽高？
                    if (ratio < (screen.x / screen.y))//16:9与 屏幕的宽高，屏幕会是多少呢？16:9 4:3 21:9和反过来的9:16 3:4 
                        mode = AspectRatioFitter.AspectMode.HeightControlsWidth;
                    else
                        mode = AspectRatioFitter.AspectMode.WidthControlsHeight;
                    
                    Debug.Log($"测试 {ratio},{screen.x},{screen.y}");
                    //我们假设屏幕是16:9的，宽高比是1.7777，ratio是4:3的，宽高比是1.3333，那么就是rotio小于screen.x/screen.y，
                }
            }
